...
- **`UniqueString` is move-only.** It uniquely owns its NFD-allocated string and frees it in the destructor, so the copy ops are deleted and only move (which nulls the source) is allowed. It is a return type only — users are not meant to construct one directly (the public `const char*` constructor assumes an NFD-allocated pointer and will `NFD_FreePath` it). Note `Result` itself has no destructor: each getter transfers ownership of `result.data` to the wrapper it returns, so calling a getter more than once on a single-pick `Result` is unsafe.
